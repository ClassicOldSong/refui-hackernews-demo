...
const CommentItem = async ({ commentId }) => {
    ...
    return (R) => (
        <div class="comment-item">
            <div class="comment-meta">
                by <a href={userUrl} target="_blank">{comment.by}</a>
            </div>
                ...
                {() => (
                    <div class="comment-children">
                        <For entries={$(() => comment.kids)}>
                            {({ item: kidId }) => <CommentItem commentId={kidId} />}
                        </For>
                    </div>
                )}
            </If>
...
const Comments = ({ storyId }) => {
    ...
    const CommentFallback = () => (R) => (
        <div class="comment-item comment-placeholder">
            <div class="comment-meta"><span class="placeholder-text"></span></div>
            <div class="comment-text"><span class="placeholder-text"></span><span class="placeholder-text"></span></div>
        </div>
    );
            ...
            <If condition={isLoadingStory}>
                ...
                {() => (
                    <If condition={storyData}>
                        {() => (
                            <>
                                <h3>Comments for: {storyData.value.title}</h3>
